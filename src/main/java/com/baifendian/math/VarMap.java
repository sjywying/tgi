...
public class VarMap {
	...
	private RoaringBitmap[] value = new RoaringBitmap[2];
	...
	*/
	public RoaringBitmap getValue(String varName) {
	...
	*/	
	public void setValue(String varName, RoaringBitmap val) {
		if (varName == null)
		...
		if (numVars == name.length) {
			...
			RoaringBitmap[] tmp2 = new RoaringBitmap[tmp1.length];
	...
	*/
	public RoaringBitmap[] getValues() {
		RoaringBitmap[] arr = new RoaringBitmap[numVars];
	...
	public void remove(String varName) {
		for (int i = 0; i < numVars; i++) {
			if (caseSensitive && name[i].equals(varName) || !caseSensitive && name[i].equalsIgnoreCase(varName)) {
				...
				numVars--;
				name[numVars] = null;
				value[numVars] = new RoaringBitmap();
				break;
			}
		}
	}
